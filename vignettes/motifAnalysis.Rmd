...
Testing for overall motif enrichment is implemented using two alternative strategies.
By default, counts of matches within the control set of sequences are assumed to follow a Poisson distribution for each PWM, and these parameters are used to perform a `poisson.test()` for each PWM, and the p-values form this test are returned.
The Z-scores returned are not used for testing and are for indicative purposes only under the Poisson model.
...
Setting the model to "iteration" instead uses an iterative approach to define the null distributions of counts and Z-scores are calculated from these values.
The returned p-values from this test are taken from the Z-scores directly, with permutation p-values also returned if preferred by the researcher.
Whilst requiring greater computational effort, fewer statistical assumptions are made beyond the Central Limit Theorem and results will be more conservative than under the Poisson approach.

```{r iter-res, eval = FALSE}
## Not run
...
ex_pwm[topMotifs] |>
  ...
  lapply(\(x) x$seq) |>
  plotOverlaps(type = "upset", min_size = 10)
```
