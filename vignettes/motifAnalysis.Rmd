...
Again, a larger set of motifs may be obtained using following code, or by customising query fileds as preferred.
...
```{r test-motif-pos}
res <- testMotifPos(matches = bm_all, mc.cores = cores) |>
  arrange(p)
head(res)
```
...
plotMatchPos(bm_all[topMotifs], abs = TRUE, binwidth = 10, geom = "col") +
  facet_wrap(~name)
plotMatchPos(bm_all[topMotifs], abs = TRUE, type = "cdf", geom = "line")
```

```{r, eval = FALSE}
topMotifs <- rownames(res_abs)[res_abs$fdr < 0.05 & res_abs$total_matches > 100]
plotMatchPos(bm_all[topMotifs], type = "heatmap", abs = TRUE, binwidth = 5) +
...
```{r}
## Need to define telomeres & centromeres for hg19
```
